<template>
  ...
  <van-floating-bubble axis="xy" icon="photo-o" v-model:offset="offset" @click="fileSelect" />

  <van-popup v-model:show="showPicker" position="bottom" class="popup">
    <van-picker :columns="columns" title="选择模型和角色" @cancel="showPicker = false" @confirm="newChat">
  ...
  <van-popup round v-model:show="show" :closeable="true" :style="{ width: '80%', display: 'flex', alignItems: 'center', justifyContent: 'center' }">
    <div class="wrapper">
      <div class="block">
        <van-uploader :after-read="afterRead" style="margin: 0 auto" :preview-image="false">
          <van-button icon="plus" type="primary">上传文件</van-button>
        </van-uploader>
        <div class="flieList">
          <van-row :gutter="[20, 20]">
            <van-col span="12" v-for="file in fileList" :key="file.url">
              <div class="grid-content">
                <!-- <el-tooltip class="box-item" effect="dark" :content="file.name" placement="top"> -->
                <van-image :src="file.url" fit="contain" v-if="isImage(file.ext)" @click.native="insertURL(file.url)" />
                <van-image :src="getFileIcon(file.ext)" fit="contain" v-else @click.native="insertURL(file.url)" />
                <van-text-ellipsis :content="file.name" />
                <!-- </el-tooltip> -->
                <div class="opt">
                  <van-button type="danger" size="mini" icon="cross" @click="removeFile(file)" circle />
                </div>
              </div>
            </van-col>
          </van-row>
...
import { processContent, randString, renderInputText, UUID, isImage, removeArrayItem } from '@/utils/libs'
        ...
        httpGet('/api/chat/history?chat_id=' + chatId.value)
          .then((res) => {
            ...
            if (data && data.length > 0) {
              ...
              // chatData.value.push({
              //   chat_id: '07313059-d8db-4509-9c07-eaa95a1ca242',
              //   content: '<p><img src="https://img-s-msn-com.akamaized.net/tenant/amp/entityid/BB1nmZ2N.img?w=640&h=836&m=6&x=318&y=233&s=203&d=203" alt=""></p>\n<p>这是什么图</p>',
              //   created_at: 1717498347,
